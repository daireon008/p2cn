...
def sendf(soc):
    soc.send('file name: '.encode())
    fname = soc.recv(size)
    fnameDecoded = fname.decode()    
    file=open(fnameDecoded,'rb')
    data=file.read()
...
def recvf(soc):
    data = soc.recv(size)
    dataDecoded = data.decode()
    user_input = input(dataDecoded)
    soc.send(user_input.encode())
    data = soc.recv(40960)
    dataDecoded = data.decode()
    ...
    # Using Select to handle multiplexing
        ...
        for sock in inready:
            if sock == s:
                ...
                clist.append(client)
                print('connected')
                client.send('Start typing'.encode())
            else:
                try:
                    data = sock.recv(size)
                    dataDecoded = data.decode()
                    if dataDecoded:
                        print(dataDecoded)
                        if dataDecoded == '\SEND_FILE':
                            sendf(client)
                        if dataDecoded== '\CLOSE_SESSION':
                            print('session ended')
                            client.close()
                            break
                        user_input = input('<Me> ')
...
def main():
    ...
    while 1:
        data = sock.recv(size)
        dataDecoded = data.decode()
        if dataDecoded.isdigit() and int(dataDecoded)>2000:
            cconnect(dataDecoded)
        user_input = input(dataDecoded)
