...
def check(client,dict,uname,pswd,c_info):
    if uname in dict and dict[uname] == pswd:
        client.send(f'Welcome back {uname}'.encode())
        corl(client,c_info,uname)
        temp = 0
    else:
...
def corl(client,c_info,uname):
    client.send('Do you want to "connect" or "listen"? '.encode())
    resp = client.recv(size)
    if resp.decode() == 'listen':
        client.send('Enter your listening port: '.encode())
        cport = client.recv(size)
        c_info[uname] = cport.decode()

#provides client_info i.e. listening port
def info(client,c_info):
    client.send('username: '.encode())
    uname = client.recv(size)
    if uname.decode() in c_info:
...
def start(client,input,dict,c_info,log):
    try:
        ...
        if dataDecoded == 'y' or dataDecoded == 'Y':    #requests new username and password if new user
            client.send('Set new username & password\nusername: '.encode())
            uname = client.recv(size)
            client.send('password: '.encode())
            pswd = client.recv(size)
            store(client,dict,uname.decode(),pswd.decode(),c_info)
        
        elif dataDecoded == 'n' or dataDecoded == 'N':   #if existing user, it verifies the username and password
            for x in range(0, 3):          #provides three attempts for user to login
                client.send('Type your username and password\nusername: '.encode())
                uname = client.recv(size)
                client.send('password: '.encode())
                pswd = client.recv(size)
                if check(client,dict,uname.decode(),pswd.decode(),c_info)==0:
                    client.send('\nType your request or type \help for help\n'.encode())
                    break
                if x == 2:
                    client.send('You have exceeded the no. of retries'.encode())        
                
        if dataDecoded == '\help':                 #if help command is given
            usage(client)    

        if dataDecoded =='\GET_CLIENT_LIST':       #send list of connected clients to user
            client.send(f'\CLIENT_LIST: {str(list(dict.keys()))}'.encode())

        if dataDecoded =='\GET_CLIENT_INFO':       #sends listening port of client
            info(client,c_info) 

        if dataDecoded == '\DISCONNECT_CLIENT':    #disconnect client if requested
            client.close()
            input.remove(client)
    ...
    while 1:
        ...
        for s in inready:
            if s == sock:
                ...
                input.append(client)
                client.send('Are you a new user? Type Y or N: '.encode())
            else:
